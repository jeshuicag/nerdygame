...
is no on-screen keypad. It answers snakehunt.py's two prompts:
  ...
  * askhead(tail, head, mis_level)  -- "shout the head": player types the
    square number they think the head is on, then Enter. A "buddy" marker
    only appears on that square once they confirm -- it is not a live
    preview. Returns the guessed square as an int.
...
Only the player's own square ever shows a number, so a child always knows
where they are without the board being cluttered with numbers everywhere.
The snake's squares reveal their numbers too, but only once mis_level (the
catch_head_mistake_level, wired in from snakehunt.py) rises above 0.
...
class SnakeHuntUI:
    ...
    def _build(self):
        ...
        self.sub_lbl = tk.Label(self.root, text="", bg=BG, fg=ACCENT,
    ...
    def _build_board(self):
        ...
        board_w = BOARD_COLS * (CELL + COL_GAP) + 24
    ...
    def _build_controls(self):
        ...
        # for what's been typed so far.
        panel = tk.Frame(self.root, bg=BG)
        panel.pack(pady=(6, 20))

        self.readout = tk.Label(panel, text="_", bg="white", fg=TEXT,
    ...
    def _confirm(self):
        ...
        if self.mode == "tail":
            ...
            # Show the buddy marker on the guessed square and hold it there,
            # visible, before the next question (right or wrong) replaces it.
    ...
    def asktail(self, place, tail, head):
        ...
        self.title_lbl.configure(text="Catch the tail!")
    ...
    def askhead(self, tail, head, mis_level, snake_len=None):
        ...
        self.snake_tail = tail
        self.snake_head = head
        self.show_snake_numbers = mis_level > 0
        self.digits = ""
        self.guess_square = None

        how_long = f" It's {snake_len} squares long." if snake_len else ""
    ...
    def moveplayer(self, player_place):
        ...
        if self._is_snake_hit(player_place):
    ...
    def _render_board(self):
        ...
        for n, cell in self._square_canvases.items():
            row, _ = self._square_rc(n)
            cell.delete("all")
            tile_color = CELL_BG_A if row % 2 == 0 else CELL_BG_B
            cell.create_rectangle(0, BADGE_H, CELL, BADGE_H + CELL,
                                  fill=tile_color, outline=CELL_BORDER, width=1)

        if self.snake_tail is not None and self.snake_head is not None:
            lo, hi = sorted((self.snake_tail, self.snake_head))
    ...
    def _draw_place_bubble(self, cell, place):
        # the only numeral always shown on the board: a big speech bubble
        # above the player, saying where they are
        cx = CELL / 2
        bw, bh = 40, 18
        top, bottom = 1, 1 + bh
        ...
        cell.create_text(cx, (top + bottom) / 2, text=str(place),
                         font=("Helvetica", 15, "bold"), fill=ACCENT)
